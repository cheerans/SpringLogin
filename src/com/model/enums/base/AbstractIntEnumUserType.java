 ...
 */
public abstract class AbstractIntEnumUserType<E extends Enum> extends AbstractEnumUserType {

    public AbstractIntEnumUserType(final Class<E> clazz) {
        super(clazz);
     }
     ...
     public Object nullSafeGet(final ResultSet resultSet, final String[] names, final Object owner) throws HibernateException, SQLException {
         if (!resultSet.wasNull()) {
             return convert(resultSet.getInt(names[0]));
         }
         return null;
     }
     ...
     public void nullSafeSet(final PreparedStatement preparedStatement, final Object value, final int index) throws HibernateException, SQLException {
         if (null == value) {
             ...
             preparedStatement.setInt(index, convert((E) value));
         }
     }

     private int convert(final E value) {
         for (final Entry<Integer, E> intToEnum : this.getConversionMap().entrySet()) {
             if (intToEnum.getValue().equals(value)) {
                 return intToEnum.getKey();
             }
         }
         return null;
     }
     
     private E convert(final int value) {
      ...
      */
     protected abstract Map<Integer, E> getConversionMap();
}